...
def mesh_arrays(mesh, nibabel=True):
    ...
    if isinstance(mesh, basestring):
       ...
       mesh = mesh
    cor = mesh.getArraysFromIntent("NIFTI_INTENT_POINTSET")[0].data
    tri = mesh.getArraysFromIntent("NIFTI_INTENT_TRIANGLE")[0].data
    return cor, tri
...
def smooth_texture(mesh, input_texture, output_texture=None, sigma=1, lsigma=1., mask=None):
    ...
    if output_texture is not None:
        if output_texture[-4:] == '.tex':
            ...
            if mask is not None:
                ...
                wdata[mask > 0] = data
            darray = GiftiDataArray().from_array(wdata.astype(np.float32), intent)
...
#####################################################################
# Gifti texture (author: Hadrien Bertrand)
#####################################################################


def get_texture(path, index):
    return read(path).darrays[index].data


def get_all_textures(path):
    list_textures = []

    for i in range(len(read(path).darrays)):
        list_textures.append(get_texture(path, i))

    return list_textures


def write_gifti(path_output, data):
    darrays_list = []
    outtexture_data = np.copy(data)  # or whatever you want to write

    darrays_list.append(GiftiDataArray().from_array(outtexture_data.astype(np.float32), 0))
    outtexture_gii = GiftiImage(darrays=darrays_list)

    write(outtexture_gii, path_output)
